...
enum ThreadMessage {
    ...
    ReleaseActiveGrab(Sender<crate::Result<()>>),
    DropThread,
}
...
impl GlobalHotKeyManager {
    ...
    pub fn release_active_grab(&self) -> crate::Result<()> {
        let (tx, rx) = crossbeam_channel::bounded(1);
        let _ = self.thread_tx.send(ThreadMessage::ReleaseActiveGrab(tx));

        match rx.recv() {
            Ok(result) => result,
            Err(error) => Err(Error::FailedToReleaseActiveGrab(error.to_string())),
        }
    }
}
    ...
    loop {
        ...
        if let Ok(msg) = thread_rx.try_recv() {
            match msg {
                ...
                ThreadMessage::ReleaseActiveGrab(tx) => {
                    let result = (|| {
                        conn.ungrab_keyboard(CURRENT_TIME)
                            .map_err(|error| Error::FailedToReleaseActiveGrab(error.to_string()))?
                            .check()
                            .map_err(|error| Error::FailedToReleaseActiveGrab(error.to_string()))?;
                        conn.flush()
                            .map_err(|error| Error::FailedToReleaseActiveGrab(error.to_string()))?;
                        for state in hotkeys.values_mut().flatten() {
                            state.pressed = false;
                        }
                        Ok(())
                    })();
