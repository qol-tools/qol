...
pub(crate) fn spawn(path: &Path, run_id: &str) -> Result<LeaseConnection> {
...
impl LeaseConnection {
    pub(crate) fn release(&mut self) -> Result<()> {
        ...
        match (write_result, wait_result) {
            ...
            (Err(write), Ok(status)) => Err(anyhow!(
                "failed to release USB host lease: {write}; helper exited with {status}"
            )),
    ...
    fn restore_driver(&self) -> Result<()> {
        ...
        for interface in &self.interfaces {
            ...
            if let Some(error) = last_error {
                return Err(error).with_context(|| format!("failed to rebind {}", interface.name));
            }
        }
        Ok(())
    }
...
fn write_sysfs(path: &Path, value: &str) -> Result<()> {
    fs::write(path, value.as_bytes()).with_context(|| format!("failed to write {}", path.display()))
}
...
pub(crate) fn validate_device(path: &Path) -> Result<()> {
