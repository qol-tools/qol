...
const PLUGIN_ID: &str = "qol-window-actions";
...
fn test_crash_recovery(guest: &mut GuestControlClient, auth: &str, window_id: &str) -> Result<()> {
    ...
    let after = wait_for_command(
        guest,
        command("/usr/bin/pgrep", &["-f", "qol-window-actions$"]),
        ACTION_TIMEOUT,
        |outcome| outcome.stdout.trim() != before,
        &format!("{PLUGIN_ID} daemon restart"),
    )?;
    if after.stdout.trim() == before {
        bail!("{PLUGIN_ID} daemon did not restart after SIGKILL");
    }
    Ok(())
}
...
fn daemon_pid(guest: &mut GuestControlClient) -> Result<String> {
    plugin_daemon_pid(
        guest,
        &["-f", "qol-window-actions$"],
        &format!("{PLUGIN_ID} daemon"),
    )
}
