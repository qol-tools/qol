...
impl DesktopCompanionRecipe {
    fn id(self) -> String {
        format!("qol-{}", self.plugin_dir)
    }
}
...
fn prepare_desktop_workflow_payload(
    ...
    if let Some(companion) = recipe.companion {
        ...
        build.arg("-p").arg(&plugin_id).arg("--bin").arg(&plugin_id);
    }
    if !verbose {
        build.arg("--quiet");
    }
    ...
    if let Some(companion) = recipe.companion {
        ...
        files.push(qol_dev_env::payload::PayloadFileSpec {
            source: binary_dir.join(crate::workspace::exe_name(&plugin_id)),
            relative_path: Path::new("plugins").join(&plugin_id).join(&plugin_id),
            executable: true,
        });
    }
    files.push(qol_dev_env::payload::PayloadFileSpec {
...
fn desktop_plugin_payload_files(
    ...
    let plugin_id = recipe.id();
    let executable_name = crate::workspace::exe_name(&plugin_id);
    let files = qol_workspace::plugin_delivery_files(
        &plugin_root,
        &[plugin_id.as_str(), executable_name.as_str()],
    )?;
    Ok(files
        .into_iter()
    ...
    fn desktop_payload_recipes_cover_registered_payload_workflows() {
        let expected = [
            ...
            (
                "qol-shot-capture",
                DesktopPayloadRecipe {
                    companion: Some(DesktopCompanionRecipe { plugin_dir: "shot" }),
                    tray_features: None,
                },
            ),
            (
