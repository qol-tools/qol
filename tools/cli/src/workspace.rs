...
fn is_qol_cli_workspace(root: &Path) -> bool {
    root.join("tools/cli/Cargo.toml").is_file()
}
...
pub(crate) fn resolve_crate_target(root: &Path, name: &str) -> Result<PathBuf> {
    ...
    let matches: Vec<&(PathBuf, Vec<String>)> = candidates
        ...
        .filter(|(_, names)| {
            names
                .iter()
                .any(|candidate| qol_conventions::plugin_id::matches(candidate, name))
        })
        .collect();
...
mod tests {
    ...
    fn resolve_crate_target_handles_qol_tray_and_siblings() {
        ...
        assert_eq!(resolve_crate_target(&repo, "qol-alt-tab").unwrap(), plugin);
    ...
    fn cargo_package_name_reads_real_package_name() {
        ...
        let cases = [
            ("alt-tab", "qol-alt-tab"),
            ("ide-checkout", "qol-ide-checkout"),
            ("pointz", "qol-pointz"),
            ("keyremap", "qol-keyremap"),
            ("lights", "qol-lights"),
        ];
    ...
    fn resolve_crate_target_includes_monorepo_plugins() {
        ...
        let resolved = resolve_crate_target(&workspace, "qol-alt-tab").unwrap();
