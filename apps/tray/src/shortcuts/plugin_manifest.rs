...
pub(super) fn reconcile<'a>(
    ...
    reconcile_with_pre_flight(config, plugins, crate::migrations::pre_flight_failed())
}

fn reconcile_with_pre_flight<'a>(
    config: &mut ShortcutsConfig,
    plugins: impl IntoIterator<Item = &'a Plugin>,
    preserve_unmatched_managed: bool,
) -> bool {
    ...
    let (mut next, seen) = reconcile_existing(existing, &desired, preserve_unmatched_managed);
...
mod tests {
    ...
    fn reconcile_adds_declared_plugin_shortcut() {
        ...
        assert_eq!(shortcut.id, "qol-cli-sessions-open");
    ...
    fn reconcile_preserves_user_disabled_state_for_existing_managed_shortcut() {
        ...
        let mut config = ShortcutsConfig {
            shortcuts: vec![Shortcut {
                id: "qol-cli-sessions-open".to_string(),
                name: "Old".to_string(),
                enabled: false,
    ...
    #[test]
    fn reconcile_preserves_unmatched_managed_shortcuts_when_pre_flight_failed() {
        let plugins = vec![plugin(
            "qol-cli-sessions",
            vec![declaration("open", "Claude")],
        )];
        let mut config = ShortcutsConfig {
            shortcuts: vec![Shortcut {
                id: "qol-alt-tab-open".to_string(),
                name: "Alt Tab".to_string(),
                enabled: true,
                export_to_launcher: true,
                source: Some(ShortcutSource::PluginManifest {
                    plugin_id: "qol-alt-tab".to_string(),
                    shortcut_id: "open".to_string(),
                }),
                action: ShortcutAction::PluginAction {
                    plugin_id: "qol-alt-tab".to_string(),
        ...
        assert!(reconcile_with_pre_flight(&mut config, &plugins, true));

        assert!(
            config
                .shortcuts
                .iter()
                .any(|shortcut| shortcut.id == "qol-alt-tab-open"),
            "managed shortcut whose plugin is absent from the partial set must survive"
        );
        assert!(config
            .shortcuts
            .iter()
            .any(|shortcut| shortcut.id == "qol-cli-sessions-open"));
    }
