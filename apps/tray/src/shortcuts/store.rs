...
mod tests {
    ...
    fn update_keeps_the_stored_source_whatever_the_request_claims() {
        let mut managed = url_shortcut("qol-lights-open", "Lights", "https://x.io");
        managed.source = Some(plugin_source("qol-lights", "open"));
        ...
        let cases = [
            (
                url_shortcut("qol-lights-open", "Renamed", "https://x.io"),
                Some(("qol-lights".to_string(), "open".to_string())),
            ),
            (forged, None),
        ];
