...
fn trace_window_action_dispatch(
    ...
    if resolved.plugin_id.as_str() != "qol-window-actions" {
        return;
    }
    let phase = input
        .get("phase")
...
mod tests {
    ...
    fn resolved() -> ResolvedAction {
        ResolvedAction {
            plugin_id: PluginId::new("qol-cli-sessions"),
    ...
    #[test]
    fn runtime_command_falls_back_to_the_theme_accent() {
        let root = tempfile::TempDir::new().unwrap();
        let _guard = crate::paths::push_test_path_root(root.path());
        ...
        let (_, value) = command
            .get_envs()
            .find(|(key, _)| *key == OsStr::new(qol_conventions::ENV_THEME_ACCENT))
            .expect("runtime action spawns must inherit an accent");
