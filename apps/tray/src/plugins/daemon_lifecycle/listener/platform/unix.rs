...
pub(in crate::plugins::daemon_lifecycle) fn bind_for_plugin(
    ...
    if !daemon_config.inherit_listener {
        return None;
    }
...
"#
    ...
    fn socket_daemon_config(socket: &str) -> DaemonConfig {
        DaemonConfig {
            ...
            port: None,
            extra_ports: Vec::new(),
            inherit_listener: true,
        }
    }

    #[test]
    fn bind_for_plugin_returns_none_without_inherit_listener() {
        let plugin = minimal_plugin();
        let mut daemon_config = socket_daemon_config("plugin-listener-test.sock");
        daemon_config.inherit_listener = false;

        assert!(
            bind_for_plugin(&plugin, &daemon_config).is_none(),
            "a daemon that has not opted into listener inheritance must never pre-bind"
        );
    }

    #[test]
    fn bind_for_plugin_binds_for_an_inheriting_plugin() {
        let _env = crate::test_support::env_lock().blocking_lock();
        let plugin = plugin_with_id("qol-alt-tab");
        ...
        assert!(
            bound.is_some(),
            "qol-alt-tab inherits its listener and must be pre-bound"
        );
        let _ = std::fs::remove_file(&socket_path);
    }
    ...
    // Regression test: qol-ide-checkout declares only a top-level `port`
    ...
    #[test]
    fn bind_for_plugin_includes_the_primary_port_for_an_inheriting_plugin() {
        let plugin = plugin_with_id("qol-alt-tab");
    ...
    #[test]
    fn bind_for_plugin_includes_extra_ports_for_an_inheriting_plugin() {
        let plugin = plugin_with_id("qol-alt-tab");
