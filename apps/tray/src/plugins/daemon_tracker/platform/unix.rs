use crate::plugins::Plugin;
use std::path::{Path, PathBuf};

struct SocketCandidate {
    path: PathBuf,
    socket: String,
}

pub(super) fn clean_stale_sockets(plugins: &[Plugin], allow_macos_temp_roots: bool) {
    for plugin in plugins {
        let Some(candidate) = socket_candidate(plugin, allow_macos_temp_roots) else {
            continue;
        };
        if has_live_listener(&candidate) {
            continue;
        }
        remove_socket(&candidate);
    }
    clean_orphan_managed_sockets(allow_macos_temp_roots);
}

fn clean_orphan_managed_sockets(allow_macos_temp_roots: bool) {
    for dir in scan_dirs() {
        clean_orphan_sockets_in(&dir, allow_macos_temp_roots);
    }
}

fn scan_dirs() -> Vec<PathBuf> {
    let mut dirs = vec![
        crate::paths::runtime_dir().join("sockets"),
        std::env::temp_dir(),
    ];
    if let Some(runtime_dir) = std::env::var_os("XDG_RUNTIME_DIR") {
        dirs.push(PathBuf::from(runtime_dir));
    }
    dirs
}

fn clean_orphan_sockets_in(dir: &Path, allow_macos_temp_roots: bool) {
    let Ok(entries) = std::fs::read_dir(dir) else {
        return;
    };
    for entry in entries.filter_map(|e| e.ok()) {
        let path = entry.path();
        let Some(candidate) = orphan_socket_candidate(&path, allow_macos_temp_roots) else {
            continue;
        };
        if has_live_listener(&candidate) {
            continue;
        }
        remove_socket(&candidate);
    }
}

fn orphan_socket_candidate(path: &Path, allow_macos_temp_roots: bool) -> Option<SocketCandidate> {
    if !is_managed_daemon_socket_path(path, allow_macos_temp_roots) {
        return None;
    }
    let socket = path.to_string_lossy().to_string();
    socket_file(SocketCandidate {
        path: path.to_path_buf(),
        socket,
    })
}

fn socket_candidate(plugin: &Plugin, allow_macos_temp_roots: bool) -> Option<SocketCandidate> {
    let candidate = existing_socket(plugin)?;
    let candidate = managed_socket(candidate, allow_macos_temp_roots)?;
    socket_file(candidate)
}

fn existing_socket(plugin: &Plugin) -> Option<SocketCandidate> {
    let daemon = plugin.manifest.daemon.as_ref()?;
    if !daemon.enabled {
        return None;
    }
    let socket = daemon.socket.as_deref()?;
    let path = crate::dev_generation::daemon_socket_path(socket);
    let socket = path.to_string_lossy().to_string();
    if !path.exists() {
        return None;
    }
    Some(SocketCandidate { path, socket })
}

fn managed_socket(
    candidate: SocketCandidate,
    allow_macos_temp_roots: bool,
) -> Option<SocketCandidate> {
    if is_managed_daemon_socket_path(&candidate.path, allow_macos_temp_roots) {
        return Some(candidate);
    }
    log::warn!(
        "Skipping unmanaged daemon socket path: {}",
        candidate.socket
    );
    None
}

fn socket_file(candidate: SocketCandidate) -> Option<SocketCandidate> {
    use std::os::unix::fs::FileTypeExt;

    let metadata = std::fs::symlink_metadata(&candidate.path).ok()?;
    if metadata.file_type().is_symlink() {
        log::warn!("Skipping symlink daemon socket path: {}", candidate.socket);
        return None;
    }
    if metadata.file_type().is_socket() {
        return Some(candidate);
    }
    log::warn!("Skipping non-socket daemon path: {}", candidate.socket);
    None
}

fn has_live_listener(candidate: &SocketCandidate) -> bool {
    use std::os::unix::net::UnixStream;

    if UnixStream::connect(&candidate.path).is_err() {
        return false;
    }
    log::info!(
        "Stale socket {} has a live listener, skipping",
        candidate.socket
    );
    true
}

fn remove_socket(candidate: &SocketCandidate) {
    log::info!("Removing stale socket: {}", candidate.socket);
    let _ = std::fs::remove_file(&candidate.path);
}

fn is_managed_daemon_socket_path(path: &Path, allow_macos_temp_roots: bool) -> bool {
    let Some(file_name) = path.file_name().and_then(|name| name.to_str()) else {
        return false;
    };
    if path.parent() == Some(crate::paths::runtime_dir().join("sockets").as_path()) {
        return file_name.ends_with(".sock");
    }
    if !file_name.starts_with("qol-") || !file_name.ends_with(".sock") {
        return false;
    }
    if starts_in_allowed_temp_root(path, allow_macos_temp_roots) {
        return true;
    }
    std::env::var_os("XDG_RUNTIME_DIR")
        .map(PathBuf::from)
        .is_some_and(|runtime_dir| path.starts_with(runtime_dir))
}

fn starts_in_allowed_temp_root(path: &Path, allow_macos_temp_roots: bool) -> bool {
    if path.starts_with(std::env::temp_dir()) {
        return true;
    }
    if !allow_macos_temp_roots {
        return false;
    }
    starts_in_macos_temp_root(path)
}

fn starts_in_macos_temp_root(path: &Path) -> bool {
    let canonical = std::fs::canonicalize(path).unwrap_or_else(|_| path.to_path_buf());
    starts_in_root(path, "/tmp")
        || starts_in_root(path, "/private/tmp")
        || starts_in_root(&canonical, "/tmp")
        || starts_in_root(&canonical, "/private/tmp")
}

fn starts_in_root(path: &Path, root: &str) -> bool {
    path.starts_with(Path::new(root))
}

pub(crate) struct ManagedRoots {
    installs_root: Option<PathBuf>,
    shared_plugins_root: Option<PathBuf>,
    dev_link_dirs: Vec<PathBuf>,
}

impl ManagedRoots {
    pub(crate) fn load() -> Self {
        Self {
            installs_root: crate::paths::installs_dir().ok(),
            shared_plugins_root: crate::paths::plugins_dir().ok(),
            dev_link_dirs: dev_link_dirs(),
        }
    }

    pub(crate) fn contains(&self, target: &Path) -> bool {
        let target = crate::file_io::canonical_or_original(target);
        self.candidate_roots()
            .iter()
            .any(|root| target.starts_with(root))
    }

    fn candidate_roots(&self) -> Vec<PathBuf> {
        let mut roots: Vec<PathBuf> = self
            .dev_link_dirs
            .iter()
            .map(|directory| crate::file_io::canonical_or_original(directory))
            .collect();

        if let Some(root) = &self.shared_plugins_root {
            roots.push(crate::file_io::canonical_or_original(root));
            roots.extend(resolved_children(root));
        }

        if let Some(root) = &self.installs_root {
            roots.push(crate::file_io::canonical_or_original(root));
            for child in resolved_children(root) {
                roots.push(crate::file_io::canonical_or_original(
                    &child.join("plugins"),
                ));
            }
        }

        roots
    }
}

fn dev_link_dirs() -> Vec<PathBuf> {
    #[cfg(feature = "dev")]
    return crate::paths::shared_config_dir()
        .map(|config_dir| {
            crate::dev::active_dev_links(&config_dir)
                .into_values()
                .collect()
        })
        .unwrap_or_default();

    #[cfg(not(feature = "dev"))]
    Vec::new()
}

fn resolved_children(directory: &Path) -> Vec<PathBuf> {
    std::fs::read_dir(directory)
        .into_iter()
        .flatten()
        .filter_map(Result::ok)
        .map(|entry| crate::file_io::canonical_or_original(&entry.path()))
        .collect()
}

pub(super) fn kill_managed_process(
    process: &super::ManagedProcess,
    roots: &ManagedRoots,
    pid_exe_path: fn(i32) -> Option<PathBuf>,
) -> bool {
    let Some(current_executable) = pid_exe_path(process.pid) else {
        return false;
    };
    if current_executable != process.executable {
        return false;
    }
    if !roots.contains(&current_executable) {
        return false;
    }
    crate::process_utils::terminate_group(process.pid, std::time::Duration::from_millis(100));
    true
}

pub(super) fn kill_from_pid_files(pid_exe_path: fn(i32) -> Option<PathBuf>) {
    let roots = ManagedRoots::load();
    let pids_dir = crate::paths::runtime_pids_dir();
    for (_, pid) in super::super::registry::tracked_pids(&pids_dir) {
        kill_pid_if_managed(&(pid as i32).to_string(), &roots, pid_exe_path);
    }
    super::super::registry::clear_all(&pids_dir);

    for path in legacy_pid_files() {
        if path.exists() {
            process_pid_file(&path, &roots, pid_exe_path);
        }
    }
}

fn shared_daemon_pids_path() -> Option<PathBuf> {
    crate::paths::shared_config_dir()
        .ok()
        .map(|path| path.join(".daemon-pids"))
}

fn legacy_pid_files() -> Vec<PathBuf> {
    let mut files = Vec::new();
    if let Some(current) = shared_daemon_pids_path() {
        files.push(current);
    }

    let Ok(installs_dir) = crate::paths::installs_dir() else {
        return files;
    };
    let Ok(entries) = std::fs::read_dir(installs_dir) else {
        return files;
    };

    files.extend(
        entries
            .filter_map(Result::ok)
            .map(|entry| entry.path().join(".daemon-pids"))
            .filter(|path| path.exists()),
    );
    files
}

fn process_pid_file(path: &Path, roots: &ManagedRoots, pid_exe_path: fn(i32) -> Option<PathBuf>) {
    let Ok(content) = std::fs::read_to_string(path) else {
        return;
    };
    for line in content.lines() {
        kill_pid_if_managed(line, roots, pid_exe_path);
    }
    let _ = std::fs::remove_file(path);
}

fn kill_pid_if_managed(line: &str, roots: &ManagedRoots, pid_exe_path: fn(i32) -> Option<PathBuf>) {
    let Ok(pid) = line.trim().parse::<i32>() else {
        return;
    };
    if !crate::process_utils::is_pid_alive(pid) {
        return;
    }
    let Some(executable) = pid_exe_path(pid) else {
        return;
    };
    if !roots.contains(&executable) {
        return;
    }
    log::info!(
        "Killing orphan daemon process: {} ({})",
        pid,
        executable.display()
    );
    crate::process_utils::terminate_group(pid, std::time::Duration::from_millis(100));
}

#[cfg(test)]
mod tests {
    use super::*;
    use std::os::unix::net::{UnixListener, UnixStream};
    use std::time::Duration;

    fn allow_macos_temp_roots() -> bool {
        cfg!(target_os = "macos")
    }

    #[test]
    fn orphan_socket_without_listener_is_removed() {
        let tmp = tempfile::TempDir::new().unwrap();
        let socket_path = tmp.path().join("qol-foo.sock");
        let listener = UnixListener::bind(&socket_path).unwrap();
        drop(listener);
        assert!(socket_path.exists(), "socket file should remain after drop");
        wait_for_listener_to_close(&socket_path);

        clean_orphan_sockets_in(tmp.path(), allow_macos_temp_roots());

        assert!(!socket_path.exists(), "orphan socket should be removed");
    }

    #[test]
    fn orphan_socket_with_live_listener_is_kept() {
        let tmp = tempfile::TempDir::new().unwrap();
        let socket_path = tmp.path().join("qol-foo.sock");
        let _listener = UnixListener::bind(&socket_path).unwrap();

        clean_orphan_sockets_in(tmp.path(), allow_macos_temp_roots());

        assert!(
            socket_path.exists(),
            "live socket should not be removed by orphan scan"
        );
    }

    #[test]
    fn non_qol_socket_is_kept() {
        let tmp = tempfile::TempDir::new().unwrap();
        let socket_path = tmp.path().join("other-foo.sock");
        let listener = UnixListener::bind(&socket_path).unwrap();
        drop(listener);

        clean_orphan_sockets_in(tmp.path(), allow_macos_temp_roots());

        assert!(
            socket_path.exists(),
            "non-qol-*.sock filenames are not managed and must be left alone"
        );
    }

    #[test]
    fn regular_file_named_qol_sock_is_kept() {
        let tmp = tempfile::TempDir::new().unwrap();
        let path = tmp.path().join("qol-bar.sock");
        std::fs::write(&path, "not a socket").unwrap();

        clean_orphan_sockets_in(tmp.path(), allow_macos_temp_roots());

        assert!(
            path.exists(),
            "regular files must not be removed even if name matches"
        );
    }

    #[test]
    fn private_runtime_socket_is_managed_for_any_plugin_filename() {
        let root = tempfile::TempDir::new().unwrap();
        let _guard = crate::paths::push_test_path_root(root.path());
        let path = crate::paths::runtime_dir()
            .join("sockets")
            .join("plugin-foo.sock");

        assert!(is_managed_daemon_socket_path(
            &path,
            allow_macos_temp_roots()
        ));
    }

    fn wait_for_listener_to_close(socket_path: &Path) {
        for _ in 0..100 {
            if UnixStream::connect(socket_path).is_err() {
                return;
            }
            std::thread::sleep(Duration::from_millis(1));
        }
        panic!("socket still accepts connections after listener drop");
    }
}
