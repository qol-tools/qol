...
fn declared_or_folder_id(manifest_path: &Path, folder: &str) -> Option<String> {
    ...
    {
        Some(id) if crate::plugins::manifest::is_valid_plugin_id(id) => Some(
...
mod tests {
    ...
    #[test]
    fn old_declared_id_registers_under_the_new_id() {
        let tmp = TempDir::new().unwrap();
        let plugins_dir = tmp.path().join("plugins");
        fs::create_dir(&plugins_dir).unwrap();
        make_plugin_dir_with_manifest(
            &plugins_dir,
            "qol-launcher",
            &valid_manifest("plugin-launcher"),
        );

        let registry = ensure_registry_initialized(tmp.path(), &plugins_dir).unwrap();
        assert_eq!(registry.entries.len(), 1);
        assert_eq!(
            registry.entries[0].id, "qol-launcher",
            "a migrated folder with an old installed manifest must register under the new id"
        );
    }
