...
mod tests {
    ...
    fn plugin_source_url_helpers() {
        ...
        assert_eq!(
            s.plugin_release_tag("qol-alt-tab", "1.2.3"),
            "qol-alt-tab-v1.2.3"
        );
    }
    ...
    fn plugin_dir_from_tree_path_table() {
        let cases: &[(&str, Option<&str>)] = &[
            ...
            ("plugins/ide-checkout/plugin.toml", Some("ide-checkout")),
    ...
    fn version_from_plugin_tag_strips_id_prefix() {
        let cases: &[(&str, &str, Option<&str>)] = &[
            ("qol-alt-tab-v1.2.3", "qol-alt-tab", Some("1.2.3")),
            ("qol-launcher-v0.1.0", "qol-launcher", Some("0.1.0")),
            (
                "qol-alt-tab-v1.2.3-beta.1",
                "qol-alt-tab",
                Some("1.2.3-beta.1"),
            ),
            ("qol-tray-v1.2.3", "qol-alt-tab", None),
            ("qol-launcher-v0.1.0", "qol-alt-tab", None),
            ("qol-alt-tab-1.2.3", "qol-alt-tab", None),
            ("qol-alt-tab-vNOTSEMVER", "qol-alt-tab", None),
            ("qol-alt-tabv1.2.3", "qol-alt-tab", None),
        ];
    ...
    fn select_release_tag_picks_max_semver_regardless_of_list_order() {
        ...
        let alt_tab = select_release_tag(tags.iter().copied(), "qol-alt-tab");
        assert_eq!(
            alt_tab,
            Some("qol-alt-tab-v1.2.3"),
            "must pick the maximum matching version, not the first match in list order"
        );

        let launcher = select_release_tag(tags.iter().copied(), "qol-launcher");
        assert_eq!(
            launcher,
            Some("qol-launcher-v0.4.0"),
            "host releases like qol-tray-vN must not match plugin selection"
        );

        let missing = select_release_tag(tags.iter().copied(), "qol-keyremap");
        assert_eq!(missing, None);
    }

    #[test]
    fn select_release_tag_orders_by_semver_not_lexicographic() {
        let tags = ["qol-launcher-v0.9.0", "qol-launcher-v0.10.0"];
        let got = select_release_tag(tags.iter().copied(), "qol-launcher");
        assert_eq!(
            got,
            Some("qol-launcher-v0.10.0"),
            "0.10.0 must beat 0.9.0 even though it sorts earlier lexicographically"
        );
    }

    #[test]
    fn selected_plugin_release_agrees_with_availability() {
        let tags = ["plugin-x-v1.2.4-rc.1", "plugin-x-v1.2.3"];
        let selected =
        ...
        assert!(
            crate::version::is_newer_version(&version, "1.2.2"),
            "selected plugin release {version} must be offered over 1.2.2"
        );
    }
    ...
    fn select_release_tag_prefers_stable_over_prerelease() {
        ...
        let got = select_release_tag(tags.iter().copied(), "qol-alt-tab");
        assert_eq!(got, Some("qol-alt-tab-v1.2.3"));
    }
    ...
    fn select_release_tag_rejects_non_semver_suffix() {
        let tags = ["qol-alt-tab-vNOTSEMVER", "qol-alt-tab-v1.0.0"];
        let got = select_release_tag(tags.iter().copied(), "qol-alt-tab");
        assert_eq!(
            got,
            Some("qol-alt-tab-v1.0.0"),
    ...
    fn select_release_tag_rejects_plugin_id_collision() {
        let tags = ["qol-alt-tab-extra-v1.0.0", "qol-alt-tab-v0.1.0"];
        let got = select_release_tag(tags.iter().copied(), "qol-alt-tab");
        assert_eq!(
            got,
            Some("qol-alt-tab-v0.1.0"),
    ...
    fn fold_collected_plugins_first_source_wins_on_id_collision() {
        let source_a = vec![
            meta("qol-alt-tab", "https://github.com/qol-tools/qol"),
            meta("qol-launcher", "https://github.com/qol-tools/qol"),
        ];
        let source_b = vec![
            meta("qol-alt-tab", "https://github.com/some-fork/qol"),
        ...
        assert_eq!(
            ids,
            vec!["qol-alt-tab", "qol-launcher", "plugin-extra"],
            "first-source-wins must preserve source-A's alt-tab and append source-B's unique entries"
        );

        let alt_tab = merged.iter().find(|m| m.id == "qol-alt-tab").unwrap();
        assert_eq!(
    ...
    fn required_release_asset_names_maps_each_binary() {
        let binaries = vec![
            BinaryDependency {
                name: "qol-alt-tab".to_string(),
                repo: "legacy/qol-tools".to_string(),
                pattern: "qol-alt-tab-{os}-{arch}".to_string(),
            },
            BinaryDependency {
                name: "qol-launcher".to_string(),
                repo: "legacy/qol-tools".to_string(),
                pattern: "qol-launcher-{os}-{arch}".to_string(),
            },
        ];
        ...
        assert!(
            names[0].starts_with("qol-alt-tab-"),
            "expected qol-alt-tab- prefix, got {:?}",
            names[0]
        );
        assert!(
            names[1].starts_with("qol-launcher-"),
            "expected qol-launcher- prefix, got {:?}",
            names[1]
        );
    }
}
