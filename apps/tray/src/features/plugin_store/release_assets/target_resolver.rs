...
mod tests {
    ...
    fn resolve_asset_pattern_linux_x86_64() {
        ...
        assert_eq!(
            resolve_asset_pattern("qol-alt-tab-{os}-{arch}", target),
            "qol-alt-tab-linux-x86_64"
        );
    }
    ...
    fn resolve_asset_pattern_windows_appends_exe() {
        ...
        assert_eq!(
            resolve_asset_pattern("qol-launcher-{os}-{arch}", target),
            "qol-launcher-windows-aarch64.exe"
        );
    }
}
