...
mod tests {
    ...
    fn source_directory_is_resolved_from_declared_identity() {
        ...
        write_plugin(repo.path(), "alt-tab", "qol-alt-tab");
        write_plugin(repo.path(), "launcher", "qol-launcher");

        let found = find_plugin_source_dir(repo.path(), "qol-alt-tab").unwrap();
    ...
    fn plugin_release_tag_builds_monorepo_tag_format() {
        let s = core_source();
        let cases = [
            ("qol-alt-tab", "1.2.3", Some("qol-alt-tab-v1.2.3")),
            (
                "qol-launcher",
                "0.1.0-beta.1",
                Some("qol-launcher-v0.1.0-beta.1"),
            ),
        ];
    ...
    fn plugin_release_tag_rejects_unsafe_input() {
        ...
        for version in bad_versions {
            assert!(
                plugin_release_tag(&s, "qol-alt-tab", version).is_err(),
                "version: {:?}",
                version
            );
        }
