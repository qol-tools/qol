...
fn validate_staged_contract(
    ...
    let staged_id = manifest.plugin.require_declared_id()?;
    let staged_uid = manifest
        .plugin
        .uid
        .clone()
...
mod tests {
    ...
    async fn extract_plugin_subdir_moves_plugin_dir_out_of_clone() {
        ...
        .await
        .unwrap();
        tokio::fs::create_dir_all(clone.join("apps").join("tray"))
            .await
            .unwrap();

        let extracted = temp.path().join("extracted");
        extract_plugin_subdir(&clone, &extracted, "qol-alt-tab")
            .await
            .unwrap();
        ...
        assert!(
            clone.join("apps").join("tray").is_dir(),
    ...
    async fn extract_plugin_subdir_errors_when_manifest_missing() {
        ...
        tokio::fs::create_dir_all(&plugin_subdir).await.unwrap();
        let extracted = temp.path().join("extracted");
        let err = extract_plugin_subdir(&clone, &extracted, "qol-alt-tab")
            .await
            .unwrap_err();
        let msg = format!("{}", err);
    ...
    fn write_manifest(dir: &Path, id: &str, uid: &str) {
        let manifest = format!(
            "[plugin]\nid = \"{id}\"\nuid = \"{uid}\"\nname = \"{id}\"\ndescription = \"\"\nversion = \"1.0.0\"\n\n[menu]\nlabel = \"x\"\nitems = []\n"
        );
    ...
    fn staged_uid_installed_under_a_different_id_is_refused() {
        ...
        assert!(
            ...
            "error must name the staged id: {msg}"
        );
        assert!(
            msg.contains("qol-lights"),
            "error must name the installed id: {msg}"
        );
    }
