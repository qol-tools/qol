...
async fn reconcile_plugins(
    ...
    for plugin in plugins {
        ...
        entry.id = super::plugins_lock::normalized_plugin_id(&entry.id);
        ...
        if current_version.as_deref() == Some(entry.version.as_str()) && !entry.version.is_empty() {
            results.push(ImportPluginResult {
                id: entry.id.clone(),
                status: "kept".to_string(),
                message: format!("already at {}", entry.version),
            });
            continue;
        }

        results.push(restore_plugin(&installer, &plugin_dir, &entry).await);
    }
...
async fn restore_exact(
    ...
    let plugin_id = super::plugins_lock::normalized_plugin_id(plugin_id);
    if exists {
