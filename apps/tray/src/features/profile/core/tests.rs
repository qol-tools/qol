...
#[test]
fn import_plugins_renames_old_lock_ids_before_dedup() {
    let bundle = ProfileImportBundle {
        plugins: vec![
            test_lock_entry("plugin-launcher", "u-launcher-old"),
            test_lock_entry("qol-launcher", "u-launcher-new"),
        ],
        ..ProfileImportBundle::default()
    };
    ...
    assert_eq!(plugins.len(), 1);
    assert_eq!(plugins[0].id, "qol-launcher");
    assert_eq!(plugins[0].uid, PluginUid::new("u-launcher-old"));
}
...
#[test]
fn import_plugins_renames_legacy_installed_plugin_ids() {
    let bundle = ProfileImportBundle {
        installed_plugins: vec!["plugin-launcher".to_string(), "qol-launcher".to_string()],
        ..ProfileImportBundle::default()
    };
    ...
    assert_eq!(
        plugins[0].repo_url,
        "https://github.com/qol-tools/qol-launcher.git"
    );
}
...
fn plugin_uid_index_refines_uid_from_installed_manifest() {
    ...
    let tmp = TempDir::new().unwrap();
    let plugins_dir = tmp.path().join("plugins");
    write_manifest_with_uid(&plugins_dir, "qol-lights", "u-real-0001", "1.0.0");

    let cases = vec![
        Case {
            name: "lock uid equals id and the manifest declares the real uid",
            entry: test_lock_entry("qol-lights", "qol-lights"),
        ...
        Case {
            ...
            entry: test_lock_entry("qol-lights", "u-lock-0001"),
...
async fn project_plugin_configs_falls_back_to_the_raw_key_directory() {
    ...
    let plugins = vec![test_lock_entry("qol-lights", "u-real-0001")];
    let uid_index = PluginUidIndex::from_plugins(&plugins_dir, &plugins, &PluginsLock::empty());
...
"#,
    ...
    write_installed_plugin_config(&plugins_dir, "qol-lights", &json!({"value": 50}));
    write_profile_plugin_config("qol-lights", &json!({"value": 73}));
    let configs = HashMap::from([
        ("u-real-0001".to_string(), json!({"value": 100})),
        ("qol-lights".to_string(), json!({"value": 73})),
    ]);
    let bundle = ProfileImportBundle {
        plugins: vec![test_lock_entry("qol-lights", "u-real-0001")],
    ...
    assert_eq!(read_profile_plugin_config("qol-lights"), None);
    assert_eq!(
        read_live_plugin_config(&plugins_dir, "qol-lights"),
        json!({"value": 100})
    );
}
...
async fn apply_import_bundle_keeps_uid_value_when_installed_plugin_is_dropped_from_lock() {
    ...
    let external_dir = external_parent.join("qol-lights");
    write_manifest_with_uid(&external_parent, "qol-lights", "u-real-0001", "1.0.0");
    write_plugin_contract(
        &external_parent,
        "qol-lights",
        r#"
...
"#,
    ...
    write_installed_plugin_config(&external_parent, "qol-lights", &json!({"value": 50}));
    std::os::unix::fs::symlink(&external_dir, plugins_dir.join("qol-lights")).unwrap();
    let bundle = ProfileImportBundle {
        plugins: vec![test_lock_entry("qol-lights", "qol-lights")],
        plugin_configs: Some(HashMap::from([
            ("u-real-0001".to_string(), json!({"value": 100})),
            ("qol-lights".to_string(), json!({"value": 73})),
        ])),
...
"#,
    ...
    let configs = HashMap::from([
        ...
        ("qol-lights".to_string(), json!({"value": 7})),
    ]);
    let bundle = ProfileImportBundle {
        plugins: vec![PluginLockEntry {
            uid: PluginUid::new("qol-lights"),
    ...
    assert!(
        error.contains("Invalid config for qol-lights"),
    ...
    assert_eq!(read_profile_plugin_config("qol-lights"), None);
    assert!(!crate::plugins::paths::config_path(&plugins_dir.join("qol-lights")).exists());
}
...
"#,
    ...
    let configs = HashMap::from([
        ("u-real-0001".to_string(), json!({"value": 100})),
        ("qol-lights".to_string(), json!({"value": "three"})),
    ]);
    let bundle = ProfileImportBundle {
        plugins: vec![test_lock_entry("qol-lights", "u-real-0001")],
        plugin_configs: Some(configs),
...
async fn apply_import_bundle_rejects_unsafe_config_keys_before_any_write() {
    ...
    .unwrap();
    let source_repo =
        create_monorepo_style_source_repo(&plugins_dir, "qol-lights", "1.0.0", None, None);
    let _source_guard = crate::features::plugin_store::source::test_seam::install(vec![
        crate::features::plugin_store::source::PluginSource::new(
            "fixture",
            source_repo.repo.as_str(),
    ...
    assert!(!plugins_dir.join("qol-lights").exists());
...
async fn build_export_bundle_prefers_uid_profile_configs_over_legacy_id_files() {
    ...
    .unwrap();
    write_installed_plugin_config(&plugins_dir, "qol-lights", &json!({"value": 42}));
    ...
    assert!(!bundle.plugin_configs.contains_key("qol-lights"));
    assert_eq!(
...
async fn plugins_lock_for_export_refines_stored_and_preserved_entries() {
    ...
    let stored = PluginsLock {
        version: CURRENT_PROFILE_VERSION,
        plugins: vec![
            test_lock_entry("qol-lights", "qol-lights"),
            PluginLockEntry {
    ...
    let lights = lock
        .plugins
        .iter()
        .find(|entry| entry.id == "qol-lights")
...
"#,
    ...
    assert_eq!(read_profile_plugin_config("qol-lights"), None);
    assert_eq!(
        read_live_plugin_config(&clean_plugins_dir, "qol-lights"),
        json!({"value": 100})
    );
}
