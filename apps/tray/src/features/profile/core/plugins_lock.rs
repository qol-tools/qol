...
pub(super) fn normalized_plugin_id(id: &str) -> String {
    qol_migrations::renamed_plugin_id(id)
        .map(str::to_string)
...
pub fn import_plugins(bundle: &ProfileImportBundle) -> Vec<PluginLockEntry> {
    if !bundle.plugins.is_empty() {
        ...
        for entry in &bundle.plugins {
            let id = normalized_plugin_id(&entry.id);
            if seen_uids.contains(entry.uid.as_str()) || seen_ids.contains(&id) {
                continue;
            }
    ...
    let mut plugins = Vec::new();
    for plugin_id in &bundle.installed_plugins {
        let id = normalized_plugin_id(plugin_id);
        if !seen.insert(id.clone()) {
            continue;
        }
        plugins.push(PluginLockEntry {
            uid: PluginUid::new(id.as_str()),
            repo_url: default_repo_url(&id),
            id,
            version: String::new(),
            platforms: None,
        });
...
pub(crate) fn sync_plugins_lock_from_plugins_with_generation<'a>(
    ...
    if crate::migrations::pre_flight_failed() {
        anyhow::bail!("refusing to rewrite the plugins lock after a failed pre-flight migration");
    }
