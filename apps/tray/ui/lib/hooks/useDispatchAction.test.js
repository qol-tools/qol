...
test('dispatch suppresses global fetch error toast for inline action errors', async () => {
    ...
    await withFetch(
        ...
        async () => {
            const action = useDispatchAction('qol-lights', 'reload');
...
test('dispatch carries row input to an action selected at activation time', async () => {
    ...
    await withFetch(
        ...
        async () => {
            const action = useDispatchAction('qol-bluetooth', null);
            await action.dispatch({ address: 'AA:BB' }, 'pair_device');
        },
    );

    assert.equal(seen[0].url, '/api/plugins/qol-bluetooth/actions/pair_device');
