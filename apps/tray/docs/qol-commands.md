...
`qol` is the local dev orchestrator (built from `tools/cli`). It is the
...
The command surface is defined in `tools/cli` and printed by `qol help` -
...
```bash
bash apps/tray/scripts/generate-icons.sh
cd apps/tray/diagram && npm install && npm run build
