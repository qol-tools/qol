...
// on the daemon's own bind-it-yourself path unchanged - see the stub below.
#[cfg(unix)]
mod imp {
    use super::{Command, DaemonConfig, Plugin};
    use crate::plugins::manifest::{NamedPort, PortProtocol};
    ...
    fn bind_extra_port(plugin: &Plugin, named_port: &NamedPort) -> Option<(String, ExtraSocket)> {
        let bound = match named_port.protocol {
            ...
            PortProtocol::Udp => {
                UdpSocket::bind(("0.0.0.0", named_port.port)).map(ExtraSocket::Udp)
            }
        };
        match bound {
...
#[cfg(not(unix))]
pub(super) fn apply_to_command(_daemon_listener: &DaemonListener, _command: &mut Command) {}
