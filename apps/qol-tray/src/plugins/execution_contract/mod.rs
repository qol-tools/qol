...
pub(crate) fn resolve_plugin_command_path_for_source(
    ...
    resolve_candidate_by_source(source, plugin_dir, command_path, &canonical_plugin_dir).or_else(
        || platform::resolve_platform_candidate(plugin_dir, command_path, &canonical_plugin_dir),
    )
}
...
pub(super) fn is_allowed_candidate(path: &Path, canonical_plugin_dir: &Path) -> bool {
    if !path.is_file() {
        return false;
    }
