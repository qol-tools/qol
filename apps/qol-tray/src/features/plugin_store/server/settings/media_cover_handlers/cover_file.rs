...
async fn cover_paths(base_dir: &Path, plugin_id: &str) -> Result<CoverPaths, Response> {
...
async fn canonical_cover_path(paths: &CoverPaths) -> Result<PathBuf, Response> {
...
async fn ensure_existing_non_symlink(path: &Path) -> Result<(), Response> {
    let metadata = metadata_or_not_found(path).await?;
    ensure_not_symlink(&metadata)
}

async fn ensure_cover_file(path: &Path) -> Result<(), Response> {
    let metadata = metadata_or_not_found(path).await?;
    ensure_not_symlink(&metadata)?;
    ensure_file(&metadata)
}

async fn metadata_or_not_found(path: &Path) -> Result<std::fs::Metadata, Response> {
    tokio::fs::symlink_metadata(path)
        .await
        .map_err(|_| cover_not_found_response())
}

async fn canonical_or_not_found(path: &Path) -> Result<PathBuf, Response> {
    tokio::fs::canonicalize(path)
        .await
        .map_err(|_| cover_not_found_response())
}

async fn ensure_cover_size(path: &Path) -> Result<(), Response> {
...
fn ensure_not_symlink(metadata: &std::fs::Metadata) -> Result<(), Response> {
...
fn ensure_file(metadata: &std::fs::Metadata) -> Result<(), Response> {
    if !metadata.is_file() {
