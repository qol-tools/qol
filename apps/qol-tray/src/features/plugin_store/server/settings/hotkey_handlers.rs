...
fn get_hotkeys_inner() -> HttpResult<Response> {
    ...
    let config = manager.load_config().map_err(|_| load_failed_response())?;
...
fn set_hotkeys_inner(body: axum::body::Bytes) -> HttpResult<Response> {
    ...
    manager
        .save_config(&config)
        .map_err(|_| save_failed_response())?;
    trigger_reload();
    Ok(hotkeys_saved_response())
}

fn hotkey_manager() -> HttpResult<HotkeyManager> {
    HotkeyManager::new().map_err(|_| manager_failed_response())
}
...
fn encode_hotkeys_json(config: &HotkeyConfig) -> HttpResult<Vec<u8>> {
    serde_json::to_vec(config).map_err(|_| serialize_failed_response())
}
