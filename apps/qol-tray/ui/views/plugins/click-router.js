...
function routeCardClick(e, index, pluginId, list, modal, actions) {
    ...
    const ctxConfig = e.target.closest('.context-config');
    if (ctxConfig) { e.stopPropagation(); modal.closeAll(); actions.openConfig(); return; }
    ...
    if (e.ctrlKey) { e.stopPropagation(); list.setSelectedIndex(index); actions.openConfig(); return; }
