...
export function LogFiltersSubPage({ slot }) {
    ...
    if (!value.scope) {
        return html`<${PageShell}
            frame=${false}
            header=${html`<${PageHeader} title="Edit Log Filters" subtitle="Open from a Dev row" />`} />`;
    }
    ...
    return html`
        <${PageShell}
            frameClassName="log-filters-frame"
            header=${html`<${PageHeader} title="Edit Log Filters" subtitle=${subtitle} />`}>
