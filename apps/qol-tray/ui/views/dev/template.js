...
export function renderDevView({ state, mergedList, getActivePluginBuildState, renderPluginBuildMeta, renderBuildResults }) {
    const pluginRows = renderPluginRows({ state, mergedList, getActivePluginBuildState, renderPluginBuildMeta });
    return `
...
function renderPluginsSection(state, mergedList, pluginRows) {
    return `
        <section class="dev-section">
            ...
            <div class="plugin-list-container">
                ${mergedList.length ? `<div class="plugin-list table-list">${pluginRows}</div>` : '<p class="empty-state">No plugins found</p>'}
            </div>
            ${renderLinkInput(state)}
        </section>
    `;
}

function renderLinkInput(state) {
...
function renderActionsSection(state, renderBuildResults) {
    return `
        <section class="dev-section">
            <h2>Actions</h2>
            ${renderReloadCard(state, renderBuildResults)}
            ${renderMockCard(state)}
        </section>
    `;
}

function renderReloadCard(state, renderBuildResults) {
    return `
