...
test('live Preact field registry covers every Rust wire field kind', () => {
    const rust = readFileSync(resolve(root, 'libs/qol-config/src/normalized.rs'), 'utf8');
...
function parseRustWireKinds(source) {
    const body = source.split('fn field_kind_name', 2)[1];
    assert.ok(body, 'field_kind_name must exist');
    return new Set([...body.matchAll(/FieldKind::\w+ => "([^"]+)"/g)].map(match => match[1]));
}
