...
pub(crate) fn open(tracker: &MonitorTracker, cx: &mut App) -> anyhow::Result<()> {
    ...
    let rows = rows_from_resolved(&resolved, &|query| match query {
...
type QueryOptions = dyn Fn(&str) -> Vec<(String, String)>;
...
pub(crate) fn rows_from_resolved(config: &ResolvedConfig, provider: &QueryOptions) -> Vec<Row> {
