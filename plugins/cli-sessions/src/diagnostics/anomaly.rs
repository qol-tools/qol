...
pub struct AnomalyRecorder {
    ring_cap: usize,
    flap_secs: u64,
    windows: HashMap<SessionId, WindowState>,
}
...
impl AnomalyRecorder {
    ...
    pub fn note(&mut self, session_id: SessionId, frame: Frame) -> Option<Anomaly> {
        let cap = self.ring_cap;
        let flap_secs = self.flap_secs;
        let state = self.windows.entry(session_id.clone()).or_default();
...
pub fn dump(dir: &Path, anomaly: &Anomaly) -> std::io::Result<PathBuf> {
    ...
    let target = dir.join(format!("{}_session-{}", stamp, anomaly.session_id.native()));
...
pub fn observe(
    ...
    if let Some(anomaly) = machine.note(session_id, frame) {
...
mod tests {
    ...
    fn session() -> SessionId {
        crate::host::kitty_session_id(1)
    }

    #[test]
    fn short_lived_needs_you_is_a_flap() {
        let mut rec = AnomalyRecorder::new(6, 9);
        assert!(rec.note(session(), frame(0, Status::Working)).is_none());
        assert!(rec.note(session(), frame(3, Status::NeedsYou)).is_none());
        let anomaly = rec
            .note(session(), frame(6, Status::Working))
    ...
    fn sustained_needs_you_is_not_a_flap() {
        let mut rec = AnomalyRecorder::new(6, 9);
        rec.note(session(), frame(0, Status::NeedsYou));
        rec.note(session(), frame(30, Status::NeedsYou));
        assert!(
            rec.note(session(), frame(60, Status::Working)).is_none(),
    ...
    fn needs_you_resolving_after_window_is_not_a_flap() {
        ...
        rec.note(session(), frame(0, Status::NeedsYou));
        assert!(
            rec.note(session(), frame(20, Status::YourTurn)).is_none(),
    ...
    fn ring_is_bounded() {
        ...
        for ts in 0..10 {
            rec.note(session(), frame(ts, Status::Working));
        }
        rec.note(session(), frame(10, Status::NeedsYou));
        let anomaly = rec.note(session(), frame(12, Status::Working)).unwrap();
        assert!(
