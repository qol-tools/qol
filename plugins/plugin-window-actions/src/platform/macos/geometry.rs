...
fn frontmost_screen() -> Result<(i32, Rect), String> {
    ...
    #[cfg(debug_assertions)]
    eprintln!(
        "[window-actions:dbg] window rect: x={} y={} w={} h={}",
    ...
    #[cfg(debug_assertions)]
    eprintln!(
        "[window-actions:dbg] screen rect: x={} y={} w={} h={}",
...
fn ax_set(pid: i32, rect: Rect) -> Result<(), String> {
    #[cfg(debug_assertions)]
    eprintln!(
        "[window-actions:dbg] ax_set pid={pid} target: x={} y={} w={} h={}",
    ...
    if let Some(after) = ax::front_window_rect(pid) {
        eprintln!(
            "[window-actions:dbg] after set: x={} y={} w={} h={}",
...
pub fn center() -> Result<(), String> {
    ...
    #[cfg(debug_assertions)]
    eprintln!(
        "[window-actions:dbg] center target: x={} y={} w={} h={}",
...
fn move_monitor(delta: i32) -> Result<(), String> {
    ...
    #[cfg(debug_assertions)]
    eprintln!(
        "[window-actions:dbg] screens ({} total): {screens:?}",
        screens.len()
    );
    if screens.len() < 2 {
        return Ok(());
