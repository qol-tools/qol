...
pub fn capture_preview_rgba(window_id: u32, max_w: usize, max_h: usize) -> Option<RgbaImage> {
    ...
    let image = conn
        ...
        .ok()?;
    let channel_order = detect_channel_order(&conn, screen_num);
    let rgba_data = x11_data_to_rgba(
        &image.data,
        geometry.width as usize,
        geometry.height as usize,
        channel_order,
    )?;
    let (data, w, h) = preview::downscale_rgba(
        &rgba_data,
        geometry.width as usize,
        geometry.height as usize,
        max_w,
    ...
    if w == 0 || h == 0 {
        ...
        Some(RgbaImage { data, width: w, height: h })
    }
}
...
pub fn capture_previews_batch_rgba(
    ...
    let mut out = Vec::with_capacity(targets.len());
    for (pos, (list_index, _window_id)) in targets.iter().copied().enumerate() {
        let Some(geo) = geometries[pos].as_ref() else {
            out.push((list_index, None));
            continue;
        };
        ...
        let result = rgba.and_then(|rgba_data| {
            let (data, w, h) = preview::downscale_rgba(
                &rgba_data,
                geo.width as usize,
                geo.height as usize,
