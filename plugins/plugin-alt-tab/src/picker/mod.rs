...
fn try_reuse_existing(req: &OpenPickerRequest, gathered: &GatheredWindows, cx: &mut App) -> bool {
    ...
    let input = reuse::LayoutInput {
        config: req.config, window_count: gathered.windows.len(),
        tracker: req.tracker, created_on_origin: origin,
    };
    let layout = reuse::compute_layout(&input, cx);
    let reuse_req = reuse::ReuseRequest {
        handle: &handle, layout: &layout, config: req.config, gathered,
    };
...
fn create_from_request(req: &OpenPickerRequest, gathered: GatheredWindows, cx: &mut App) {
    let create_req = create::CreateRequest {
        config: req.config, tracker: req.tracker,
        last_window_count: req.last_window_count.clone(),
        icon_cache: req.icon_cache.clone(), current: req.current,
    };
...
fn try_cycle_selection(
    ...
    handle.update(cx, |view, window: &mut Window, cx| -> bool {
...
fn finalize_reuse(
    ...
    let _ = handle.update(cx, |view, _window, cx| {
        view.delegate.update(cx, |state, _| state.insert_previews(previews));
        cx.notify();
    });
    let icon_req = IconFillRequest {
        handle, windows: gathered.windows.clone(), icon_cache: icon_cache.clone(),
    };
...
pub(crate) mod state {
    ...
    impl PickerState {
        ...
        pub(crate) fn apply_config(
            &mut self, config: &AltTabConfig, card_color: u32, card_opacity: f32,
        ) {
        ...
        pub(crate) fn mark_minimized(&mut self, window_id: u32) {
            let Some(idx) = self.windows.iter().position(|w| w.id == window_id) else { return };
        ...
        fn move_in_grid(&mut self, direction: GridDirection, columns: usize) {
            ...
            let current = self.selected_index.unwrap_or(0).min(total.saturating_sub(1));
    ...
    pub(crate) fn push_focus_hint(win: &WindowInfo) {
        ...
        eprintln!(
            ...
            idx, win.width as i32, win.height as i32,
    ...
    pub(crate) fn find_containing_monitor(monitors: &[qol_plugin_api::MonitorBounds], x: f32, y: f32) -> Option<usize> {
    ...
    impl Grid {
        fn new(columns: usize, total: usize) -> Self {
            ...
            Self { cols, rows: (total + cols - 1) / cols, total }
        }
    ...
    fn grid_left(current: usize, row: usize, g: &Grid) -> usize {
        ...
        if current > row_start { current - 1 } else { current }
    }

    fn grid_right(current: usize, row: usize, g: &Grid) -> usize {
        if current + 1 < g.row_end(row) { current + 1 } else { current }
    }
