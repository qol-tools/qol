...
unsafe fn ax_open_window_list(pid: i32) -> *const c_void {
    ...
    CFRelease(app);
    if err != 0 || val.is_null() { return std::ptr::null(); }
    val
}
...
unsafe fn ax_check_subrole(win: *const c_void, attr: *const c_void) -> bool {
    ...
    if val.is_null() { return true; } // include when AX unavailable — safe default
...
unsafe fn ax_read_title(win: *const c_void, attr: *const c_void) -> String {
    let val = ax_copy_attr(win, attr);
    if val.is_null() { return String::new(); }
...
impl Drop for AxAttrs {
    fn drop(&mut self) {
        unsafe {
            CFRelease(self.subrole); CFRelease(self.id);
            CFRelease(self.title); CFRelease(self.minimized);
        }
    }
}
...
pub(super) fn ax_windows(pid: i32) -> Option<(HashMap<u32, AxWindowMeta>, Vec<AxWindowMeta>, usize)> {
    ...
    if wins_val.is_null() { return None; }
    ...
    drop(attrs);
    #[cfg(debug_assertions)]
    eprintln!("[alt-tab/ax] ax_windows pid={} id_map={} all_meta={} accepted={}", pid, id_map.len(), all_meta.len(), accepted);
    Some((id_map, all_meta, accepted))
}

fn collect_ax_window_meta(
    wins_val: *const c_void, count: isize, attrs: &AxAttrs,
    ...
    for i in 0..count {
        ...
        if !unsafe { ax_check_subrole(win, attrs.subrole) } { continue; }
        accepted += 1;
        let meta = AxWindowMeta {
            title: unsafe { ax_read_title(win, attrs.title) }.trim().to_string(),
...
fn pid_dedup_from_ax(ax: &Option<(HashMap<u32, AxWindowMeta>, Vec<AxWindowMeta>, usize)>) -> PidDedup {
    match ax {
        ...
        Some((_, _, accepted)) => PidDedup {
            ax_ids: HashSet::new(), ax_meta: HashMap::new(),
...
fn emit_deduped(windows: Vec<CgWindow>, dedup_info: &HashMap<i32, PidDedup>) -> Vec<CgWindow> {
    ...
    for mut win in windows {
        ...
        if !should_keep(&win, dedup, *count) { continue; }
...
pub(crate) unsafe fn ax_find_window(
    pid: i32, cg_window_id: u32, title_hint: &str,
) -> *const c_void {
    let wins_val = ax_open_window_list(pid);
    if wins_val.is_null() { return std::ptr::null(); }
    ...
    let result = scan_for_match(wins_val, count, cg_window_id, title_hint, id_attr, title_attr);
...
unsafe fn scan_for_match(
    wins_val: *const c_void, count: isize,
    cg_window_id: u32, title_hint: &str,
    id_attr: *const c_void, title_attr: *const c_void,
) -> *const c_void {
    ...
    for i in 0..count {
        ...
        if win.is_null() { continue; }
        if first_win.is_null() { first_win = CFRetain(win); }
        if id_match.is_null() {
...
unsafe fn pick_best_match(
    id_match: *const c_void, title_match: *const c_void,
    first_win: *const c_void, count: isize,
) -> *const c_void {
    if !id_match.is_null() {
