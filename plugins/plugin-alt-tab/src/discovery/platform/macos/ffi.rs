...
extern "C" {
    ...
    pub(crate) fn CGWindowListCreateImage(
        screen_bounds: CGRect, list_option: u32, window_id: u32, image_option: u32,
    ) -> CGImageRef;
...
extern "C" {
    ...
    fn CFStringCreateWithBytes(
        alloc: *const c_void, bytes: *const u8, num_bytes: isize,
        encoding: u32, is_external: bool,
    ) -> *const c_void;
    fn CFStringGetLength(s: *const c_void) -> isize;
    fn CFStringGetMaximumSizeForEncoding(length: isize, encoding: u32) -> isize;
    fn CFStringGetCString(
        s: *const c_void, buffer: *mut u8, buffer_size: isize, encoding: u32,
    ) -> bool;
}

pub(crate) fn cfstr(s: &[u8]) -> *const c_void {
    unsafe {
        CFStringCreateWithBytes(std::ptr::null(), s.as_ptr(), s.len() as isize, K_CF_STRING_ENCODING_UTF8, false)
    }
}
...
pub(crate) fn dict_get_i32(dict: CFDictionaryRef, key: *const c_void) -> Option<i32> {
    ...
    let ok = unsafe { CFNumberGetValue(val, K_CF_NUMBER_INT32_TYPE, &mut result as *mut i32 as *mut c_void) };
...
pub(crate) fn dict_get_f64(dict: CFDictionaryRef, key: *const c_void) -> Option<f64> {
    ...
    let ok = unsafe { CFNumberGetValue(val, K_CF_NUMBER_FLOAT64_TYPE, &mut result as *mut f64 as *mut c_void) };
    if ok { Some(result) } else { None }
}

pub(crate) fn dict_get_rect(dict: CFDictionaryRef, key: *const c_void) -> Option<(f64, f64, f64, f64)> {
    let val = dict_get_value(dict, key)?;
    ...
    if ok { Some((rect.origin.x, rect.origin.y, rect.size.width, rect.size.height)) } else { None }
}
...
pub(crate) fn cfstring_to_string(s: *const c_void) -> Option<String> {
    ...
    if !unsafe { CFStringGetCString(s, buf.as_mut_ptr(), buf.len() as isize, K_CF_STRING_ENCODING_UTF8) } {
        return None;
    }
...
pub(crate) fn cfnumber_to_u32(num: *const c_void) -> Option<u32> {
    if num.is_null() { return None; }
    let mut val: i64 = 0;
    let ok = unsafe { CFNumberGetValue(num, K_CF_NUMBER_SINT64_TYPE, &mut val as *mut i64 as *mut c_void) };
