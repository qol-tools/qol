...
impl PlatformQueries for MacQueries {
    ...
    fn focused_window_bounds(&self) -> Option<Bounds<Pixels>> {
        ...
        let key_bounds_x = cg_helpers::cfstr(b"X");
        let key_bounds_y = cg_helpers::cfstr(b"Y");
        let key_bounds_w = cg_helpers::cfstr(b"Width");
        let key_bounds_h = cg_helpers::cfstr(b"Height");
        ...
        for i in 0..count {
            ...
            let Some(win_pid) = cg_helpers::dict_get_i32(dict, key_pid) else {
                continue;
            };
            ...
            let (Some(x), Some(y), Some(w), Some(h)) = (
                cg_helpers::dict_get_f64(bounds_dict, key_bounds_x),
                cg_helpers::dict_get_f64(bounds_dict, key_bounds_y),
                cg_helpers::dict_get_f64(bounds_dict, key_bounds_w),
                cg_helpers::dict_get_f64(bounds_dict, key_bounds_h),
            ) else {
                continue;
            };
            if w > 0.0 && h > 0.0 {
