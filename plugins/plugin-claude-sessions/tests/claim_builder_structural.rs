...
fn fixture_jsonl_path() -> std::path::PathBuf {
    std::path::PathBuf::from(format!(
        "/home/user/.claude/projects/-home-user-repos-private-qol-tools-workspace/{VALID_UUID}.jsonl"
    ))
}
...
fn builder_rejects_jsonl_without_uuid_basename() {
    let bad = Path::new("/home/user/.claude/projects/-home-user/not-a-uuid.jsonl");
...
fn builder_rejects_extension_mismatch() {
    let bad = Path::new(&format!(
        "/home/user/.claude/projects/-home-user/{VALID_UUID}.txt"
    ))
    .to_path_buf();
    let res = build_claim(&bad);
    assert!(
