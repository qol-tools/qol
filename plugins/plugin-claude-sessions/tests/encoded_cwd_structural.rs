...
//! path has every `/` replaced with `-`, so `/Users/kaho` becomes
//! `-Users-kaho`. This test locks that invariant; if Claude ever changes
...
fn encode_cwd_replaces_slashes_with_dashes() {
    let cwd = Path::new("/Users/kaho/repos/private/qol-tools/workspace");
    assert_eq!(
        encode_cwd(cwd),
        "-Users-kaho-repos-private-qol-tools-workspace"
    );
}

#[test]
fn encode_cwd_preserves_dots_and_underscores() {
    let cwd = Path::new("/Users/kaho/repos/foo.bar/baz_qux");
    assert_eq!(encode_cwd(cwd), "-Users-kaho-repos-foo.bar-baz_qux");
}
...
fn encode_cwd_output_satisfies_template_regex_charset() {
    ...
    let cwd = Path::new("/Users/kaho/.claude/projects-test");
