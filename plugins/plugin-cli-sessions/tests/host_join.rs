use qol_terminal_sessions::kitty::{backend_id, parse_ls};
...
fn parse_ls_extracts_panes_with_shell_integration() {
    let panes = parse_ls(SAMPLE, backend_id())
        .expect("parse")
        .sessions(backend_id());
    assert_eq!(panes.len(), 3, "all panes returned");

    let claude = panes.iter().find(|pane| pane.id.native() == "10").unwrap();
    ...
    let codex = panes.iter().find(|pane| pane.id.native() == "11").unwrap();
    assert_eq!(codex.reported_cmd.as_deref(), Some("codex"));
    assert!(codex.foreground_basenames.contains(&"codex".to_string()));

    let shell = panes.iter().find(|pane| pane.id.native() == "12").unwrap();
