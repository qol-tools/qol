# CLI Sessions Plugin for QoL Tray

[![tests](https://github.com/qol-tools/plugin-cli-sessions/actions/workflows/tests.yml/badge.svg)](https://github.com/qol-tools/plugin-cli-sessions/actions/workflows/tests.yml)
[![lint](https://github.com/qol-tools/plugin-cli-sessions/actions/workflows/lint.yml/badge.svg)](https://github.com/qol-tools/plugin-cli-sessions/actions/workflows/lint.yml)

An always-on-top overview of live CLI sessions (Claude Code, Codex, any command) for [QoL Tray](https://github.com/qol-tools/qol-tray). Each kitty pane is colored by workflow state so you can see at a glance which session wants attention.

## Quick start

Install from the [qol-tray](https://github.com/qol-tools/qol-tray) plugin store, or build from source:

```bash
git clone https://github.com/qol-tools/plugin-cli-sessions
cd plugin-cli-sessions
make build
```

## License

PolyForm Noncommercial 1.0.0
