...
impl AdaptivePoller {
    ...
    pub(crate) fn reconfigure(&mut self, min: Duration, max: Duration, strategy: Box<dyn PollStrategy>) {
        self.min = min;
        self.max = max;
...
mod tests {
    ...
    fn poller_ramps_down_then_up() {
        ...
        assert_eq!(interval, ms(500), "should reach max after enough idle ticks");
    }
