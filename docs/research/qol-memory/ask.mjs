...
const ALIASES = process.env.QOL_MEMORY_ALIASES_DISABLE === "1" ? new Map() : loadAliases(join(ASK_BASE, "concept-aliases.json"));
...
if (existsSync(skillsIndexPath)) {
  try {
    skillsIndex = JSON.parse(readFile(skillsIndexPath, "utf8"));
  } catch {
    skillsIndex = null;
  }
}
