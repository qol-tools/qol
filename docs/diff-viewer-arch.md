...
2026-08-13: time-warp scrub. The commit filmstrip is a time axis: changing the selection plays the transition between the two rendered diffs as motion. qol-diff gains transition::TransitionPlan, a pure per-file mapping between two flattened line lists; identity is the shared file position (old lines keyed by old_line_no, new lines keyed by new_line_no, which coincide for adjacent commits), with a text-equality fallback for keyless lines (old added against new removed). Carried lines stay put, modified lines cross-fade in place, added lines fade in, and removed lines render as a ghost layer that drifts up and fades out.
2026-08-13: the transition plays as gpui element animations (200 ms, quint ease-out) driven by the animation clock: no tick loop, no timers, and no git spawns. The new diff is the immediate source of truth, rows are always built from the new state and ghosts carry no hitboxes, so clicking and typing during the animation act on the new state with no input lockout. An uncached scrub keeps the current pane until facts land and then plays the transition. The scrubber gains an on_select selection-change hook so key and drag selection changes reach the view immediately; the 50 ms result poll remains as the safety net.
