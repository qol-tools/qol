...
use qol_theme::{
    alt_tab_preview_plane_runtime, css, dark_accent_preset, dark_theme, dark_theme_with_accent_key,
    resolve_surface_override, runtime_dark_theme, PickerSurfacePalette, ThemeMode,
    DARK_ACCENT_PRESETS, DARK_REFERENCE, DARK_SYSTEM, DARK_TRAY_INTERNAL, LIGHT_REFERENCE,
    LIGHT_SYSTEM, PROD_ACCENT_KEY, TEXT_SCALE, THEME_COLOR_SENTINEL,
};
use std::{
    fs,
    path::Path,
...
fn gpui_surfaces_only_use_type_scale_sizes() {
    let workspace = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let roots = ["libs/qol-gpui/src", "plugins"];
    let mut violations = Vec::new();

    for root in roots {
        for path in rust_sources(&workspace.join(root)) {
            if path.components().any(|part| part.as_os_str() == "examples") {
                continue;
            }
            ...
            for (index, line) in contents.lines().enumerate() {
                let compact = line
                    ...
                    .filter(|character| !character.is_whitespace())
                    .collect::<String>();
                let Some(rest) = compact.split("text_size(px(").nth(1) else {
                    continue;
                };
                let Some(raw) = rest.split("))").next() else {
                    continue;
                };
                let Ok(size) = raw.parse::<f32>() else {
                    continue;
                };
                if !TEXT_SCALE.contains(&size) {
                    let relative = path.strip_prefix(&workspace).unwrap_or(&path);
                    violations.push(format!("{}:{} uses {size}", relative.display(), index + 1));
                }
            }
        }
    }
