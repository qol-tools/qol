...
pub fn macos_installed_apps(app_dirs: &[PathBuf], spotlight: Spotlight<'_>) -> Vec<InstalledApp> {
    ...
    match spotlight {
        Spotlight::Disabled => {}
        Spotlight::All => candidates.extend(crate::platform::spotlight_app_paths(&[])),
        Spotlight::Roots(roots) => {
            candidates.extend(crate::platform::spotlight_app_paths(roots));
        }
    }
...
fn launcher_entry(path: &Path) -> Option<AppEntry> {
    ...
    Some(AppEntry {
        ...
        exec: vec!["open".to_string(), path.display().to_string()],
