...
use super::components::{
    display_layout_stage, display_layout_tile, paint_settings_selection, rail_caption,
    rail_caption_height, settings_action_affordance, settings_action_spinner, settings_description,
    settings_label, settings_label_group, settings_message, settings_page, settings_query_spinner,
    DisplayLayoutTile, SettingsFeedback, SettingsGroupHeader, SettingsRow, SettingsSelectValue,
    SettingsToggle,
};
use super::display_layout::{nudge_step, DisplayLayoutState};
...
impl SettingsPanelView {
    ...
    fn pick_display_layout_mode(&mut self, pick: usize) {
        if let Some(state) = self.level_mut().display_layout.as_mut() {
    ...
    fn display_layout_viewport(&self) -> (f32, f32) {
        ...
        let fallback_width = super::PANEL_WIDE_WIDTH - 2.0 * qol_theme::SPACE_PAD;
        (fallback_width, super::PANEL_DISPLAY_LAYOUT_HEIGHT)
    }

    fn sync_display_layout_card(&mut self, query: &str) {
        if self.stack.len() <= 1 {
            return;
        }
    ...
    fn open_display_layout_card(&mut self, row_index: usize, cx: &mut Context<Self>) {
        let Some(row) = self.level().rows.get(row_index) else {
            return;
        };
    ...
    fn value_color(&self, index: usize) -> u32 {
        ...
        match &self.level().rows[index].control {
            ...
            RowControl::DisplayLayout(_) => self.palette.label_text,
    ...
    fn render_display_layout_card(&self, cx: &mut Context<Self>) -> Vec<AnyElement> {
        let Some(state) = self.level().display_layout.as_ref() else {
            return Vec::new();
        };
        ...
        .inset_0();
        let mut stage =
            display_layout_stage(palette, super::PANEL_DISPLAY_LAYOUT_HEIGHT).child(stage_bounds);
        match fit {
            Some(fit) => {
...
impl SettingsPanelView {
    fn body_items(&self, cx: &mut Context<Self>) -> Vec<AnyElement> {
        ...
        } else if let Some(draft_body) = self.render_object_array_card_draft(cx) {
    ...
    fn render_content(
        ...
        if !self.rail_is_open() {
            ...
            return base.child(self.render_deck(depth, card, slide, drawer));
        }
        ...
        let card_layer = match depth {
            ...
            _ => {
                let deck = self
                    .render_deck(depth, card, slide, drawer)
                    .absolute()
                    .right_0()
                    .top_0()
                    .bottom_0();
    ...
    fn render_hint_bar(&self, cx: &App) -> Div {
        if self.current_source_is_custom() && self.body_has_focus() {
            if let Some(hints) = self.custom_view().and_then(|custom| custom.hint_pairs(cx)) {
                return self
                    .kit
                    .hint_bar()
                    .children(
                        hints
                            .into_iter()
                            .map(|(key, label)| self.kit.hint(key, label)),
                    )
                    .child(div().flex_1())
...
fn card_enter_hint(level: &Level) -> Option<&'static str> {
    if level.display_layout.is_some() {
        return Some(match level.selected {
...
"#;
    ...
    fn display_layout_state() -> DisplayLayoutState {
        ...
        state.set_viewport(
            624.0,
            super::super::PANEL_DISPLAY_LAYOUT_HEIGHT,
    ...
    fn a_display_layout_card_level_exposes_five_rows_and_starts_on_the_stage() {
        ...
        assert_eq!(super::card_enter_hint(&card), Some("edit"));
    }
    ...
    fn cancel_leaves_the_root_state_untouched() {
        ...
        assert!(pop_level(&mut stack).is_some());
