...
fn stage_into(
    ...
    for spec in files {
        ...
        set_file_mode(&destination, spec.executable)?;
...
#[cfg(not(unix))]
fn set_file_mode(path: &Path, _executable: bool) -> Result<()> {
...
fn make_tree_read_only(root: &Path) -> Result<()> {
    ...
    for path in directories_deepest_first(root)? {
...
mod tests {
    ...
    fn verification_detects_post_stage_tampering() {
        ...
        {
            use std::os::unix::fs::PermissionsExt;
            fs::set_permissions(&payload.root, fs::Permissions::from_mode(0o755)).unwrap();
            fs::set_permissions(payload.root.join("file"), fs::Permissions::from_mode(0o644))
                .unwrap();
        }
        #[cfg(not(unix))]
        {
            let path = payload.root.join("file");
