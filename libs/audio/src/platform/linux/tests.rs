...
#[test]
fn default_names_report_presence_and_absence() {
    ...
    let server = FakeServer::start(vec![typed(info), typed(protocol::ServerInfo::default())]);
    let mut connection = connect(&server);
    let present = devices::default_name(&mut connection, Direction::Output).expect("default sink");
    assert_eq!(present, Some("alsa_output.pci".to_owned()));
    let absent = devices::default_name(&mut connection, Direction::Output).expect("absent");
